...
class PageFr5 extends Component {
  state = {
    image: null,
    name:'',
    show: false,
  ...
  render() {
        ...
        <View style={styles.container}>
          <View style={styles.question2StackStack}>
            <View style={styles.question2Stack}>
              <Text style={styles.question2}>التقط صورة أو حمّلها.</Text>
              <ImageBackground
              ...
              >
            <TouchableOpacity onPress={() =>{this.props.navigation.navigate('Pagear4')}}>
    <Icons name={'arrow-back'}  color='#fff'  style={styles.icon1} />
           ...
           <EntypoIcon name="camera" style={styles.icon2}
             onPress={() =>{this.props.navigation.navigate('Camar')}}
            ></EntypoIcon>
          
          </View>
          <View style={styles.uploadRow}>
         
            <Text style={styles.upload}>تحميل</Text>
            
            <Text style={styles.camera}>تصوير</Text>
            <Text style={styles.orTapHere}  onPress={()=>this._pickImage()} >أو اضغط هنا</Text>
          </View>
  ...
  getPermissionAsync = async () => {
    if (Constants.platform.ios) {
      ...
      if (status !== 'granted') {
        alert('عذرا ، نحن بحاجة إلى أذونات لفة الكاميرا لإنجاز هذا العمل!');
      }
    }
  }
  ...
  _pickImage = async () => {
    ...
    bottom:hp('15%'), width:wp('100%'), left:wp('-40%'), height:hp('8%')
      },
    });
    
    export default PageFr5;
